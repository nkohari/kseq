...
describe("Ident", () => {
  ...
  describe("getDepth()", () => {
    ...
    it("returns the correct depth", () => {
      let ident = Ident.parse("1#0.foo/1.foo/2.foo");
  ...
  describe("compare()", () => {
    ...
    it("when x is identical to y, returns 0", () => {
      let x = Ident.parse("1#0.foo/1.foo/2.foo");
      let y = Ident.parse("1#0.foo/1.foo/2.foo");
    ...
    it("when x and y are same depth, and x[1].digit < y[1].digit, returns -1", () => {
      let x = Ident.parse("1#0.foo/1.foo/2.foo");
      let y = Ident.parse("1#0.foo/2.foo/2.foo");
      assert.equal(x.compare(y), -1);
    });
    ...
    it("when x and y are same depth, and x[1].digit > y[1].digit, returns 1", () => {
      let x = Ident.parse("1#0.foo/2.foo/2.foo");
      let y = Ident.parse("1#0.foo/1.foo/2.foo");
      assert.equal(x.compare(y), 1);
    });
      
    it("when x = [1,3] and y = [1,2,3], returns 1", () => {
      let x = Ident.parse("1#1.foo/3.foo");
      let y = Ident.parse("1#1.foo/2.foo/3.foo");
      assert.equal(x.compare(y), 1);
    });
    
    it("when x = [1,2,3] and y = [1,3], returns -1", () => {
      let x = Ident.parse("1#1.foo/2.foo/3.foo");
      let y = Ident.parse("1#1.foo/3.foo");
      assert.equal(x.compare(y), -1);
    });    
